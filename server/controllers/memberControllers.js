// ? server/controllers/memberControllers.js
...
export const getAllMembers = async (req, res) => {
    try {
        ...
        // Syntax MySQL2 Promise: const [rows]
...
export const renewMember = async (req, res) => {
    ...
    try {
        ...
        const invoiceNumber = `INV-${Date.now()}`; // Generate Invoice simpel
        const trxSql = `
            INSERT INTO transactions (invoice_number, member_id, package_id, amount, type, transaction_date)
            VALUES (?, ?, ?, ?, 'Renew', NOW())
        `;
        await db.query(trxSql, [invoiceNumber, id, package_id, amount]);

        res.json({ success: true, message: "Perpanjangan Berhasil!", invoice: invoiceNumber });
